...
type IStream[T any, R any] interface {
	...
	Filter(f ConditionalFunc[T], threads ...int) IStream
	...
	//            available CPUs will be the number that determines the amount of go channels to be used. If order matters,
	//            best combine it with a `SortBy`. Only needs to be provided once per stream.
	Except(f ConditionalFunc[T], threads ...int) IStream
	...
	//            available CPUs will be the number that determines the amount of go channels to be used. If order matters,
	//            best combine it with a `SortBy`. Only needs to be provided once per stream.
	Map(f func(a T) R, threads ...int) IStream
	...
	//
	// - f:       The matching function to be used.
	IfAllMatch(f ConditionalFunc[T]) IThen
	...
	//
	// - f:       The matching function to be used.
	IfNotAllMatch(f ConditionalFunc[T]) IThen
